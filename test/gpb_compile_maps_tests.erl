...
map_type_with_mapfields_as_maps_option_test() ->
    ...
    M1 = compile_iolist(["message m1 { map<string,fixed32> a = 1; };"],
                        [mapfields_as_maps]),
    B1 = M1:encode_msg(#m1{a = #{"x" => 17}}),
    #m1{a = #{"x" := 17}} = M1:decode_msg(B1, m1),
    unload_code(M1),

    %% messages are maps, but map<_,_> are 2-tuples
    M2 = compile_iolist(["message m1 { map<string,fixed32> a = 1; };"],
                        [msgs_as_maps, {mapfields_as_maps, false}]),
    B2 = M2:encode_msg(#{a => [{"x",17}]}, m1),
    #{a := [{"x", 17}]} = M2:decode_msg(B2, m1),
    unload_code(M2).
